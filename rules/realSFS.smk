...
rule theta_stat_SW:
  ...
  shell:
    ...
    module load singularity/2.x
    module load angsd/0.938
    singularity exec /apps/uibk/angsd/0.938/angsd.sandbox /opt/angsd/misc/thetaStat do_stat saf/POPS/{wildcards.POPS}_{wildcards.IND}_{wildcards.MinDepth}_{wildcards.MaxDepth}.GL2.thetas.idx -win 10000 -step 1000 -outnames saf/POPS/{wildcards.POPS}_{wildcards.IND}_{wildcards.MinDepth}_{wildcards.MaxDepth}.GL2.theta_stats_Window10kb 2> {log}
    """
...
rule Fst_index:
  ...
  shell:
    ...
    module load singularity/2.x
    module load angsd/0.938
    singularity exec /apps/uibk/angsd/0.938/angsd.sandbox /opt/angsd/misc/realSFS fst index saf/POPS/{wildcards.POP1}*.GL2.saf.idx saf/POPS/{wildcards.POP2}*.GL2.saf.idx -sfs {input.pop_pair} -fstout saf/POPS/{wildcards.POP1}_vs_{wildcards.POP2} 2> {log.pop_pair}
    """
...
rule Fst_global:
  ...
  log:
    pop_pair = 'log/saf_POPS/{POP1}_vs_{POP2}.fst_Global.log'
  threads: 12
  message:
...
rule Fst_windows:
  ...
  shell:
    ...
    module load singularity/2.x
    module load angsd/0.938
    singularity exec /apps/uibk/angsd/0.938/angsd.sandbox /opt/angsd/misc/realSFS fst stats2 saf/POPS/{wildcards.POP1}_vs_{wildcards.POP2}.fst.idx -win 10000 -step 1000 > saf/POPS/{wildcards.POP1}_vs_{wildcards.POP2}.fst.Window10kb 2> {log.pop_pair}
    """
