rule PCAngsd_LDpruned:
  input:
    touched = 'angsd/{sets}/LDpruned_angsd_GL{GL}_minInd{IND}_maf{minMaf}_minDepth{MinDepth}_maxDepth{MaxDepth}_{Minweight}Minweight.done'
  output:
    touch('pcangsd/{sets}/PCAngsd_LDpruned_angsd_GL{GL}_minInd{IND}_maf{minMaf}_minDepth{MinDepth}_maxDepth{MaxDepth}_{Minweight}Minweight.done')
  log: 'log/{sets}/PCAngsd_LDpruned_angsd_GL{GL}_minInd{IND}_maf{minMaf}_minDepth{MinDepth}_maxDepth{MaxDepth}_{Minweight}Minweight.log'
  threads: 24
  message:
  ...
  shell:
    ...
    for i in {{2..10}}; do
       pcangsd -beagle angsd/{wildcards.sets}/LDpruned_angsd_GL{wildcards.GL}_minInd{wildcards.IND}_maf{wildcards.minMaf}_minDepth{wildcards.MinDepth}_maxDepth{wildcards.MaxDepth}_{wildcards.Minweight}Minweight.beagle.gz -o pcangsd/{wildcards.sets}/PCAngsd_LDpruned_angsd_GL{wildcards.GL}_minInd{wildcards.IND}_maf{wildcards.minMaf}_minDepth{wildcards.MinDepth}_maxDepth{wildcards.MaxDepth}_{wildcards.Minweight}Minweight -minMaf {wildcards.minMaf} -admix -admix_K $i -e 10 -threads {threads} 2> {log}
    done
    """
