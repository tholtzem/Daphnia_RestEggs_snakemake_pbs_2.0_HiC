...
rule get_fixedSites:
  input:
    ...
    pop1 = 'list/pop_list/longispina7.csv',
    pop2 = 'list/pop_list/galeata.csv',
    hybrids = 'list/pop_list/hybrids_LC.csv'
  output:
    fixed_sites = 'ancestry/LC/new/{prefix}_hf.DP{genoDP}_fixed_sites.txt',
    report = 'ancestry/LC/new/{prefix}_hf.DP{genoDP}_get_fixedSites_sites.report'
  log:
...
riule plot_fixedSites:
  input:
    fixed_sites = 'ancestry/LC/new/{prefix}_hf.DP{genoDP}_fixed_sites.txt',
  output:
    svg = 'ancestry/LC/new/{prefix}_hf_DP{genoDP}_fixed_sites_100percent_thinned1000bp.svg',
    report = 'ancestry/LC/new/{prefix}_hf_DP{genoDP}_plot_fixed_sites_100percent_thinned1000bp.report'
  log:
    'log/plot_fixedSites_{prefix}_hf_DP{genoDP}_fixed_sites_80percent.log'
